resource "aws_autoscaling_group" "employee" {
  ...
  target_group_arns   = [aws_lb_target_group.employee.arn]
...
resource "aws_autoscaling_policy" "employee" {
  name                   = "web-asg-policy"
  autoscaling_group_name = aws_autoscaling_group.employee.name
...
resource "aws_launch_template" "employee" {
  ...
  user_data = base64encode("${file("./src/employee/userdata.sh")}"
  )
