...
resource "aws_instance" "bastion" {
  ...
  user_data = <<-EOF
    ...
    su - ec2-user -c 'aws s3 cp s3://${aws_s3_bucket.app.id}/ ~/wsi-repo --recursive'
    ...
    su - ec2-user -c 'cd ~/wsi-repo && git commit -m "day2"'
    ...
    su - ec2-user -c 'cd ~/wsi-repo && git remote add origin ${aws_codecommit_repository.commit.clone_url_http}'
